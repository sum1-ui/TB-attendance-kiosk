...
import { backupDBToS3 } from "./backup";
...
if (enabledActions.backupDBToS3) {
    schedule.scheduleJob("5 22 * * *", async () => {
        try {
            const db = await open({
                filename: DB_PATH,
                driver: sqlite3.cached.Database,
            });
            await backupDBToS3(db);
...
const createWindow = async () => {
    ...
    ipcMain.on("backupDBToS3", async () => {
        try {
            await backupDBToS3(db);
            await dialog.showMessageBox(mainWindow, {
                title: "Success",
                message: "Database backed up to S3 successfully",
            });
        } catch (err) {
            dialog.showErrorBox("Error", err.toString());
        }
